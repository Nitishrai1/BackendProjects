export default function Alltask({ todos }) {
  ...
  {statusItems.map((item) => (
    <div
      key={item.title}
      className={`flex flex-1 w-40 flex-row items-center p-4 bg-white rounded-lg shadow-md transition-transform transform hover:scale-105`}
    >
      <div className={`w-3 h-3 ${item.color} rounded-full`} />
      <div className="flex flex-row justify-center items-center ml-2">
        <p className="text-sm text-gray-500 mr-2">{item.title}</p>
        <p className={`text-lg font-bold ${item.color.replace("bg-", "text-")}`}>
          {item.count}
        </p>
      </div>
    </div>
